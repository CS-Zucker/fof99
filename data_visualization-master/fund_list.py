...
from database_sql import fundDatabase
...
def fund_list_insert_db(fund_dict):        
    funddb = fundDatabase()
    funddb.createTable()
    funddb.insertData(table='fund_list', my_dict=fund_dict)

def fund_list_select_db(fund_key):        
    funddb = fundDatabase()
    select_tuple_list = funddb.selectData(table='fund_list', my_key=fund_key)
    fund_code = []
    fund_name = []
    fund_type = []
    for select_tuple in select_tuple_list:
        fund_code.append(select_tuple[0])
        fund_name.append(select_tuple[1])
        fund_type.append(select_tuple[2])
    return {'code': fund_code, 'name': fund_name, 'type': fund_type}


if __name__ == '__main__':
    # print(len(get_fund_list()))
    # print(get_fund_list())
    print(fund_list_select_db('9702'))